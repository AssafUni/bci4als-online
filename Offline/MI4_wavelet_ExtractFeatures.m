...
MIData(:,12:13,:) = [];
ICA_data = MIData;                                              % just to allocate memory and save run time
for i = 1:size(MIData,1)
...
% for i = 1:size(norm_MIData,1)
%     label = targetLabels(1,i);
%     t = 1:length(norm_MIData(i,1,:));
%     figure('Name', strcat('C3 - ', num2str(label)));
%     plot(t, squeeze(norm_MIData(i,1,:)), t, squeeze(final_ica(i,1,:)));
%     legend({'c3','c3 ica'});
%     figure('Name', strcat('C4 - ', num2str(label)));
%     plot(t, squeeze(norm_MIData(i,2,:)), t, squeeze(final_ica(i,2,:)));
%     legend({'c4','c4 ica'});
%     figure('Name', strcat('CZ - ', num2str(label)));
%     plot(t, squeeze(norm_MIData(i,3,:)), t, squeeze(final_ica(i,3,:)));
%     legend({'cz', 'cz ica'})
% end
